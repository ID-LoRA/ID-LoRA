#!/usr/bin/env python3
"""
ID-LoRA two-stage HQ inference pipeline.

Uses Res2s second-order sampler for stage 1 (fewer steps, higher quality).
Stage 1: Distilled LoRA (strength 0.25) + ID-LoRA + rescale guidance.
Stage 2: Distilled LoRA (strength 0.5) only, 2x spatial upsample.

Usage
-----
# Single-sample inference
python scripts/inference_two_stage_hq.py \
...
from ltx_core.components.guiders import CFGGuider, CFGStarRescalingGuider, STGGuider
...
DEFAULT_UPSAMPLER = "models/ltx-2.3-spatial-upscaler-x2-1.1.safetensors"
DEFAULT_DISTILLED_LORA = "models/ltx-2.3-22b-distilled-lora-384.safetensors"
...
DEFAULT_VIDEO_RESCALE = 0.45
DEFAULT_AUDIO_RESCALE = 1.0
DEFAULT_IDENTITY_GUIDANCE_SCALE = 3.0
DEFAULT_AV_BIMODAL_SCALE = 3.0
...
# ---------------------------------------------------------------------------
# Two-stage HQ ID-LoRA pipeline
# ---------------------------------------------------------------------------

class IDLoraTwoStagesHQPipeline:
    """
    Two-stage HQ pipeline for audio identity transfer with ID-LoRA.

    Stage 1: Res2s sampler at height x width, distilled LoRA (0.25) + ID-LoRA + rescale guidance.
    Stage 2: 2x spatial upsample, distilled LoRA (0.5), no ID-LoRA.
    ...
    def __init__(
        ...
        quantize: bool = False,
        fp8: bool = False,
        distilled_strength_s1: float = DISTILLED_STRENGTH_STAGE_1,
        distilled_strength_s2: float = DISTILLED_STRENGTH_STAGE_2,
        stg_scale: float = 0.0,
        ...
        av_bimodal_scale: float = DEFAULT_AV_BIMODAL_SCALE,
        video_rescale: float = DEFAULT_VIDEO_RESCALE,
        audio_rescale: float = DEFAULT_AUDIO_RESCALE,
    ):
        ...
        self._stg_blocks = stg_blocks if stg_blocks is not None else [28]
        ...
        self._video_rescale = video_rescale
        self._audio_rescale = audio_rescale

        quantization = QuantizationPolicy.fp8_cast() if fp8 else None

        distilled_lora_s1: tuple[LoraPathStrengthAndSDOps, ...] = ()
        distilled_lora_s2: tuple[LoraPathStrengthAndSDOps, ...] = ()
        if distilled_lora_path and Path(distilled_lora_path).exists():
            distilled_lora_s1 = (LoraPathStrengthAndSDOps(
                path=distilled_lora_path,
                strength=distilled_strength_s1,
                sd_ops=LTXV_LORA_COMFY_RENAMING_MAP,
            ),)
            distilled_lora_s2 = (LoraPathStrengthAndSDOps(
                path=distilled_lora_path,
                strength=distilled_strength_s2,
                sd_ops=LTXV_LORA_COMFY_RENAMING_MAP,
            ),)
            print(f"Distilled LoRA: {distilled_lora_path} (s1={distilled_strength_s1}, s2={distilled_strength_s2})")
        else:
            print(f"WARNING: distilled LoRA not found at {distilled_lora_path!r}")
        ...
        self._stage_1_ledger = ModelLedger(
            ...
            loras=(*tuple(ic_loras), *distilled_lora_s1),
            quantization=quantization,
        )

        self._stage_2_ledger = self._stage_1_ledger.with_loras(distilled_lora_s2)
    ...
    def load_stage_1_models(self):
        ...
        print("Loading stage-1 transformer (ID-LoRA + distilled)...")
        if self._quantize:
    ...
    def load_stage_2_models(self):
        print("Loading stage-2 transformer (distilled LoRA only)...")
        if self._quantize:
            from ltx_trainer.quantization import quantize_model
    ...
    def __call__(
        ...
        video_cfg = CFGStarRescalingGuider(video_guidance_scale) if self._video_rescale > 0 else CFGGuider(video_guidance_scale)
        audio_cfg = CFGStarRescalingGuider(audio_guidance_scale) if self._audio_rescale > 0 else CFGGuider(audio_guidance_scale)
        ...
        if getattr(self, "_stage_1_transformer", None) is None:
            print("Reloading stage-1 transformer...")
            if self._quantize:
                from ltx_trainer.quantization import quantize_model
                x0_model = self._stage_1_ledger.transformer()
                quantize_model(x0_model.velocity_model, "int8-quanto")
        ...
        # Stage 1: Res2s denoising at target resolution
        ...
        def stage_1_denoise(video_state: LatentState, audio_state: LatentState, sigmas: torch.Tensor, step_index: int):
            sigma = sigmas[step_index]
            print(f"  S1 {step_index+1}/{total_s1} sigma={sigma.item():.4f}", flush=True)

            vs, as_ = video_state, audio_state
        ...
        print(f"Stage 1 (HQ/Res2s): generating at {s1_h}x{s1_w} ({num_inference_steps} steps)...")
        ...
        # Stage 2: Res2s refine at 2x resolution with distilled LoRA only
        ...
        stepper_s2 = Res2sDiffusionStep()

        def stage_2_denoise(video_state: LatentState, audio_state: LatentState, sigmas: torch.Tensor, step_index: int):
            sigma = sigmas[step_index]
        ...
        print(f"Stage 2 (HQ/Res2s): refining at {s2_h}x{s2_w} ({len(sigmas_s2)-1} steps)...")
        ...
        del self._stage_2_transformer
        self._stage_2_transformer = None
        cleanup_memory()

        tiling_config = TilingConfig.default()
        generator_decode = torch.Generator(device=self.device).manual_seed(seed)
        video_chunks = list(vae_decode_video(
            video_state.latent, self._stage_2_ledger.video_decoder(), tiling_config, generator_decode,
        ))
        # Each chunk is [F, H, W, C] uint8 [0,255]; concat on frames dim
        decoded_frames = torch.cat(video_chunks, dim=0)
        video_tensor = decoded_frames.permute(0, 3, 1, 2).float().cpu() / 255.0

        audio_decoder = self._stage_2_ledger.audio_decoder()
        vocoder = self._stage_2_ledger.vocoder()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="ID-LoRA two-stage HQ inference: Res2s sampler with rescale guidance",
    ...
    parser.add_argument("--video-rescale", type=float, default=DEFAULT_VIDEO_RESCALE,
                        help="Video CFG rescale (0 = standard CFG)")
    parser.add_argument("--audio-rescale", type=float, default=DEFAULT_AUDIO_RESCALE,
    ...
    parser.add_argument("--distilled-strength-s2", type=float, default=DISTILLED_STRENGTH_STAGE_2,
...
def main() -> None:
    ...
    print("=" * 80)
    print("ID-LoRA Inference -- Two-Stage HQ Pipeline (Res2s)")
    print("=" * 80)
    ...
    print("\nCreating two-stage HQ pipeline...")
    pipeline = IDLoraTwoStagesHQPipeline(
        ...
        fp8=args.fp8,
        distilled_strength_s1=args.distilled_strength_s1,
        distilled_strength_s2=args.distilled_strength_s2,
    ...
    for prompt in tqdm(all_prompts, desc="Encoding prompts"):
        results = encode_prompts(
            ...
            model_ledger=pipeline._stage_1_ledger,
        )
        ctx_p, ctx_n = results
    ...
    for i, sample in enumerate(tqdm(samples, desc="Generating")):
        ...
        audio_sr = getattr(pipeline, "_vocoder_sr", 24000)
        save_video(
